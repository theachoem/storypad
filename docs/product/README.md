# Product Documentation

Product-focused documentation for marketing, planning, and strategic decisions.

## Contents

- **[Overview](overview.md)** – Product identity, mission, platforms, monetization
- **[Deals Strategy](deals-strategy.md)** – Promotional deals, pricing strategy, implementation
- **[Metrics](metrics.md)** – User base, growth, app store performance
- **[Marketing Guide](marketing-guide.md)** – Brand positioning, messaging, content strategy
- **[User Personas](user-personas.md)** – Target audience, use cases, pain points

## Quick Reference

### Product Identity

- **Mission:** Simplicity + powerful features + customization
- **Audience:** General users who write and prefer minimalism
- **Brand:** Minimalist, creative

### Key Stats (Oct 2025)

- 100K+ downloads
- 3-5K monthly active users
- 4.5-star rating
- Available on Android & iOS

### Monetization

- Free core app (no ads, no subscriptions)
- Optional $1.99 add-ons (4 current: Voice Journal, Templates, Relaxing Sounds, Period Calendar)
- Promotional deals (30-50% off) via Remote Config
- **No bundles** – Individual deals scale infinitely as catalog grows

### Community

- [Reddit: r/StoryPad](https://www.reddit.com/r/StoryPad/)
- [X: @storypadapp](https://x.com/storypadapp)
- [Website: storypad.me](https://storypad.me)
- [GitHub: theachoem/storypad](https://github.com/theachoem/storypad)

## Use Cases

**When creating marketing content:**
→ Use [Marketing Guide](marketing-guide.md) for messaging and positioning

**When planning product deals:**
→ Reference [Deals Strategy](deals-strategy.md) for implementation and best practices

**When analyzing growth:**
→ Review [Metrics](metrics.md) for user base and growth trends

**When defining target audience:**
→ Reference [User Personas](user-personas.md) for audience characteristics

**When pitching to press/partners:**
→ Start with [Overview](overview.md) for product summary
