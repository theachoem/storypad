...
extension ListICloudFileConvert on List<ICloudFile> {
  ...
  Future<CloudFiles> _iCloudFileTofile(ICloudFile file, String containerId, [List<String> path = const []]) async {
    int index = path.indexWhere((e) => e.contains(Uri.decodeComponent(file.relativePath.replaceAll('%20', ' '))));
    String? newFilePath;
    ...
    String fileName = (file.relativePath.split("/").last.toString().split(".").first).replaceAll('%20', ' ');

    await logFileSize(newFilePath ?? file.relativePath);
    return CloudFiles(file.hashCode, fileName, newFilePath ?? file.relativePath, file.creationDate, file.sizeInBytes,
  ...
  Future<String?> downloadFileFromICloud(String relativePath, String newPath, String containerId) async {
    try {
