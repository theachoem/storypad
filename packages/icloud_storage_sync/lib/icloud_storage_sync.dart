...
class IcloudStorageSync {
  ...
  Future<List<CloudFiles>> getCloudFiles({required String containerId}) async {
    try {
      ...
      return await response.toFile(path.map((e) => e.toString()).toList(), containerId);
  ...
  Future<void> uploadMultipleFileToICloud({required String containerId, required List<File> files}) async {
    for (var file in files) {
      await upload(
        ...
        onProgress: (progress) async {
          ...
          debugPrint("File: ${file.path}, First Progress: $firstProgress, Last Progress: $lastProgress");
  ...
  Future<void> deleteMultipleFileToICloud({required String containerId, required List<String> relativePathList}) async {
  ...
  Future<void> rename({
    ...
    await move(
      ...
      toRelativePath: Uri.decodeComponent(relativePath).substring(0, relativePath.lastIndexOf('/') + 1) + newName,
    );
  }
  ...
  static bool _validateFileName(String name) =>
      !(name.isEmpty || name.length > 255 || RegExp(r"([:/]+)|(^[.].*$)").hasMatch(name));
}
