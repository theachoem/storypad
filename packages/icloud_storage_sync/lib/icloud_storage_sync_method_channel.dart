...
class MethodChannelIcloudStorageSync extends IcloudStorageSyncPlatform {
  ...
  Future<List<ICloudFile>> gather({
    ...
    final eventChannelName = onUpdate == null ? '' : _generateEventChannelName('gather', containerId);

    if (onUpdate != null) {
      // Create an event channel for updates
      await methodChannel.invokeMethod('createEventChannel', {'eventChannelName': eventChannelName});
      ...
      final stream = gatherEventChannel
          ...
          .map<List<ICloudFile>>((event) => _mapFilesFromDynamicList(List<Map<dynamic, dynamic>>.from(event)));

      onUpdate(stream);
    }

    // Invoke the gather method on the native side
    final mapList = await methodChannel.invokeListMethod<Map<dynamic, dynamic>>('gather', {
  ...
  Future<void> upload({
    ...
    if (onProgress != null) {
      ...
      final stream =
          uploadEventChannel.receiveBroadcastStream().where((event) => event is double).map((event) => event as double);
  ...
  List<ICloudFile> _mapFilesFromDynamicList(List<Map<dynamic, dynamic>>? mapList) {
  ...
  String _generateEventChannelName(String eventType, String containerId, [String? additionalIdentifier]) => [
        'icloud_storage',
        'event',
        eventType,
        containerId,
