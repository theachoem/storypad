...
class ICloudBackupService {
  ...
  static func deleteFile(relativePath: String, result: @escaping FlutterResult) {
    performCoordinatedOperation(result: result) { dataRoot in
      let url = dataRoot.appendingPathComponent(relativePath)
  ...
  private static func performCoordinatedOperation(
    ...
    operation: @escaping (URL) throws -> Void
  ) {
    guard let dataRoot = dataRootURL else {
      result(containerUnavailableError())
      return
    ...
    DispatchQueue.global(qos: .userInitiated).async {
      ...
      coordinator.coordinate(writingItemAt: dataRoot, options: .forMerging, error: &coordinatorError) { _ in
        do {
          try operation(dataRoot)
        } catch {
          operationError = error
        }
      }
